# -*- coding: utf-8 -*-
"""
Created on Sun Apr  5 15:24:35 2020
...
for s in subjects:
    for trial in range(1,ntrials+1): 
        ...
        Y=np.sin(4*time)+np.sin(7*time)+np.sin(9*time)
        ...
        ax[0].plot(time, accX)   
        ax[0].plot(time[ipk],accX[ipk], linestyle='', marker='o', markerfacecolor='None', markeredgecolor='r')   
        ...
        for i in range(0,len(cycle_starts)):
                        ...
                        ps.integ(accX[cycle_starts[i]:cycle_ends[i]],time[cycle_starts[i]:cycle_ends[i]])
                        VV,PP=  ps.integ(accX[cycle_starts[i]:cycle_ends[i]],time[cycle_starts[i]:cycle_ends[i]])
                        ps.Ttest(VV,PP)

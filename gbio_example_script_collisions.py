...
import os
from Positions import integ as Positions
...
def make_plots(beginning,The_end,Name='alex',position=False,Delai=False,add=False,zoom=False,chock_number=1,Trial=0,Type='',noreturn=True):
    ...
    for s in subjects:
        ...
        for trial in range(1,4): 
            if trial==Trial or Trial==0:
                ...
                fig = None ; ax = None
                if zoom and (not add) and (not Delai) and (not position):
                    fig = plt.figure(figsize = [3,12]) 
                elif (not add) and (not Delai) and (not position):
                    fig = plt.figure(figsize = [15,7])
                if add or Delai or position:  
                    if hb == 'haut': 
                        if block == 'avec':
                ...
                if (not zoom) and (not add) and (not Delai) and (not position):
    ...
    if add and noreturn:
        ...
        ST.MoyDeplacement(dis,Name,Type)  
    elif position:
        return dis